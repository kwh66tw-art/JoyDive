...
- 本地 HEAD：`c23b9e1 docs: ARCHITECTURE.md 補修兩處漏網的過時敘述（總指揮驗收發現）`
  （2026-09-13 更新；中間經過 `2039378`／`70853a8`／`0a4d77e` 全盤複查修正／
  `c23b9e1` 家族總指揮補修 ARCHITECTURE.md 剩餘漏網處）。
- ✅ **已確認 push，`HEAD == origin/main == c23b9e1`**（`git rev-parse HEAD
  origin/main` 實測一致）。
...
5. ✅（2026-09-13）**`.xcarchive` 清理——PM 已裁示「先不動」，這是刻意不做，
   不是忘記做**：`~/Library/Developer/Xcode/Archives/2026-06-17/` 下實際是
   **3 個** `.xcarchive`（`ls -la` mtime 皆為 6/17 當天早上 09:30／10:39／
   11:06，**確認是原記錄漏登第三份，不是事後新增**，非本輪產生的新檔案）。
   刪除為不可逆操作，**PM 裁示維持現狀，不排入下一次開工**；下一個 session
   看到這 3 個檔案不需要重新提出「要不要清」，除非 PM 主動改變裁示。
6. ✅（2026-09-13）文件全盤複查已完成，`CLAUDE.md`／`ARCHITECTURE.md`／
   `V1_2_BACKLOG.md` 的既有文件債已修正並經家族總指揮獨立複驗，**驗收發現
   `ARCHITECTURE.md` 有兩處漏網（檔頭摘要:4、ASCII 架構圖:19-21，同一份檔案
   裡還在講同一件過時的事）已由總指揮補修（commit `c23b9e1`，已 push）**。
   **教訓**：修一處過時敘述前要先問「同一份檔案裡還有幾處在講同一件事」，
   不能只改被指名/被發現的那一處。**下一次開工不需要重複這輪複查**，除非
   又有新的架構/狀態變動未同步。
...
7. 地圖 VoiceOver 無障礙改造、Vary-by-Plural 顯示 bug
   ...
   獨立工作，未排期。
8. ~~`.xcarchive` 清理決策~~——**PM 已裁示先不動**（2026-09-13，見上方
   「進行中的決策」#5），不需要再排這一步，除非 PM 改變裁示。
...
### `ARCHITECTURE.md`（新發現，已修正——不在總指揮原始回報的四項內；後續有補修）
...
- 「JD2Core/Algorithm」節原寫「現在僅存 `DiveReplayEngine.swift`」，與
  ...
  `CLAUDE.md` 改對了但 `ARCHITECTURE.md` 沒同步跟上，本輪一併修正
  （commit `0a4d77e`）。
- 🔴 **驗收發現本輪只改了一處，同一份檔案裡還有兩處在講同一件過時的事**：
  檔頭摘要（:4）與 ASCII 架構圖（:19-21）都還寫著 `DiveReplayEngine.swift`，
  「最後更新」日期也還停在 2026-07-28（本輪明明編輯了這個檔案）。已由家族
  總指揮補修（commit `c23b9e1`，已 push，`origin/main` 已同步）。
  **教訓（同 HANDOFF 陷阱提醒 #1，且是 2026-09-12～13 期間第四次同形狀失誤）**：
  修一處過時敘述前要先問「同一份檔案裡還有幾處在講同一件事」，不能只改
  被指名/被發現的那一處；家族層/單一文件改對了也不代表同一份文件內其餘
  段落都同步，逐一 grep 確認比信任「應該已經改過」可靠。
...
- #10「2 個 `.xcarchive` 待送審過關再清」：**已查證**送審觸發條件早已滿足
  （2026-07-29 上架）但清理動作未執行；`ls -la` 實測
  `~/Library/Developer/Xcode/Archives/2026-06-17/` 底下實際是**3 個**
  `.xcarchive`，mtime 皆為 6/17 當天早上（09:30／10:39／11:06）——**確認
  是原記錄漏登第三份（`11.03` 那份），不是事後新增**（mtime 排除了「事後
  新增」的可能，不需要再查來源）。**PM 已裁示「先不動」，這是刻意不做，
  不是忘記做**——不再排入下一次開工，除非 PM 改變裁示（見上方「進行中的
  決策」#5）。
- #6、#24 經核對**與實際相符**，未動。
