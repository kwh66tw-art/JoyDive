...
| 2 | 2026-07-17 | **ImportCoordinator 批次匯入去重漏洞**：`deduplicateDives` 原本只比對資料庫既有記錄的靜態快照（`let existing = try database.fetchAllDives()` 後用 `.filter`），同一批次（甚至單一檔案）內部彼此重複的日誌，因當下都還沒寫入資料庫，會互相漏檢、全數通過並寫入 | `JD2Core/Importers/ImportCoordinator.swift` `deduplicateDives()` | `JD2UltraPhone/Import/ImportCoordinator.swift:231-241`（邏輯相同：`let existing = ...`＋`.filter`，未動態累積） | ✅ **已修（2026-09-09 查證更正）** | **狀態原記「已確認未修」，實測為過時**——ultra 兩條匯入路徑（`ImportCoordinator.swift:82` 背景解析版、`:110` 直接版）現在都收斂到 `DiveImportKit.ImportBatchProcessor.dedupe`，該實作是**逐筆比對＋動態累積**（`existing.append(fp)` 在迴圈內），Kit 註解並直接點名原始 bug：「原始 bug：`existing` 陣列在迴圈中從不更新，只用 `.filter` 對照呼叫當下的靜態快照」。⇒ **本條缺陷已隨 R-071／R-074 收斂順帶修復**，不需另行處理。📌 Logbook 側的 `deduplicateDives` 現為非生產路徑（2026-09-09 稽核發現④），故本列的 Logbook 半已失去意義。原修復手法保留備查：改為逐筆比對＋動態把每筆已確認非重複的日誌併入比對陣列（`existing.append(dive)`），與 `DiveLogDatabase.importFromJSON`（JD2-Logbook 既有正確實作，備份還原用）手法一致，可直接參考 |
...
| 7 | 2026-07-25 | **DiveAnalysisView trimix 潛水狀態列整列被免責聲明取代**（非 bug 已知限制的延伸修復，供 Ultra 比對）：原本邏輯是 `if replay.decoDataUnavailable { 顯示免責聲明 } else { calloutRow + 組織艙圖 }`——但 trimix 潛水的 `ReplayPoint` 其實 Time/Depth/Temp 都有算（單純深度/時間/溫度重放，不需要減壓生理），只有 Ceiling/No Deco/組織艙資料因 DiveKit 沒有氦氣隔室模型才缺，原本的寫法卻連本來就有的資料都被免責聲明整個蓋掉。JD2-Logbook 已修復：`calloutRow` 固定 5 欄排版，只有 Ceiling/No Deco 兩欄缺資料時顯示「—」佔位，Time/Depth/Temp 三欄照常互動；組織艙圖才用免責聲明取代。Ultra 端 `DiveAnalysisView.swift` 的 `calloutRow` 是同一批移植過去的程式碼（見上方 #6），**未查證是否有相同結構**（本次未逐行核對 Ultra 原始碼），如果同樣是「trimix 直接整個 branch 換成免責聲明」的寫法，會有一樣的問題 | `JD2-Logbook/JD2-Logbook/Views/Logbook/DiveAnalysisView.swift` `body`／`calloutRow` | `JD2UltraPhone/UI/Logbook/DiveAnalysisView.swift`（未查證，同批次移植來源，建議核對） | ❓ 待查證 | 此為 App 層 UI bug，非 DiveKit／DiveImportKit 範圍，不受單一戰場規則限制；根本限制（trimix 氦氣組織負荷計算）仍是 DiveKit 缺口，見 `_JD2-family/decisions/2026-07-18_trimix減壓計算缺口.md` |
...
| 8 | 2026-07-26 | **本地錯誤 enum 複製 Kit 樣板文字，Kit 端改了這裡沒跟著改**：延續 #5 記錄的「Kit 錯誤需逐 case 轉回 App 本地同名 error enum」模式——`DiveImportKitAdapter.mapImportKitError` 把 `DiveImportKit.DiveLogImportError`（家族層）轉成本地 `DiveLogImportError`（`JD2Core/Importers/DiveLogImporter.swift`），該 adapter 自己的註解明講「訊息內容與 Kit errorDescription 一致」。但 2026-07-25 家族總指揮把 Kit 的 6 個樣板文字從中文改成英文（`R-2026-07-25-DiveImportKit錯誤訊息未本地化.md` 修法 1）時，**沒有連帶檢查/修到 Logbook 這個本地複製的 enum**，導致 Kit 端已經修好、但使用者實際看到的錯誤訊息（走 `mapImportKitError` 轉換後顯示）仍是本地複製版的舊中文樣板——表面上像是「Kit 修復沒生效」，實際上是兩份重複定義各自為政。已在 Logbook 這邊同步修正（本地 enum 樣板文字比照 Kit 改英文）。~~**Ultra 若之後採用 Kit（見 #5，目前仍 ❓ 待查證/尚未採用）需要注意**~~：**2026-07-28 更正**——Ultra 其實**已經有**同款本地複製的 `DiveLogImportError`（`JD2UltraPhone/Import/DiveLogImporter.swift`，非「尚未採用」），一樣殘留舊版中文樣板，是這個維護陷阱的第二個實際發生的實例，不是理論疑慮。只要沿用「本地同名 enum + adapter 逐 case 轉換」這個模式，Kit 端任何 errorDescription 樣板文字異動都要記得手動同步到本地複製版，這個模式本身沒有機制防止兩邊漂移，屬於架構設計就存在的維護陷阱，不是一次性 bug | `JD2Core/Importers/DiveLogImporter.swift` `DiveLogImportError.errorDescription` | `JD2UltraPhone/Import/DiveLogImporter.swift` `DiveLogImportError.errorDescription` | ✅ 已修復（家族總指揮直接處理，2026-07-28，commit `9fa2689`；由新增的 `check_family_drift.sh` Section 10 機械化掃描抓到，非人工複查發現） | 已補上 `check_family_drift.sh` Section 10（掃描三 App 是否殘留 DiveImportKit v0.4.0 前的舊版中文錯誤樣板），未來 Kit 樣板再異動時會自動抓到殘留，不必再靠人工記得逐一比對三個 App。根本解法（不複製樣板文字，只轉換 case 結構＋直接沿用 Kit 的 errorDescription 字串）仍待評估是否值得重構，見 `DiveImportKit/CLAUDE.md` 鐵律一節 |
| 9 | 2026-08-22 | **重放引擎收斂進 DiveKit，Logbook 端已移除自有複製**（非 bug，記錄給 Ultra 端採用時參考）：`JD2Core/Algorithm/DiveReplayEngine.swift` 已刪除，全面改呼叫 `DiveKit.DiveReplayEngine.replayChain(target:precedingDives:personal:)`。Ultra 端的 `JD2UltraPhone/UI/Logbook/DiveReplay.swift` 是同一份邏輯的另一個複製（本 repo 那份的來源），同樣應該收斂。**Logbook 端採用時踩到的兩點**：(a) DiveKit `ReplayResult` 是 public struct 但成員初始化器仍是 internal，消費端造不出空實例，`@State` 必須改成 Optional（`ReplayResult?`）；(b) `DiveInput.maxDepthMeters` 一定要帶（保守上界截斷的方形剖面估算依據），不帶會讓幾天前的手動輸入紀錄誤觸 P6、把整個組織艙顯示關掉。**Logbook 與 Ultra 的能力落差**：`recordedSeriesIndex` 在 Logbook 恆為 nil（無 `diveNumberInSeries` 欄位，P4 被跳過），Ultra／immersion 有此欄位、應該要傳，不得照抄 Logbook 的 nil | `JD2Core/Algorithm/DiveReplayInput.swift`（映射＋96h 前導潛水查詢）／`Views/Logbook/DiveAnalysisView.swift` | `JD2UltraPhone/UI/Logbook/DiveReplay.swift`（未逐行核對本次版本，為同批移植來源） | ❓ 待查證 | 規格：`_JD2-family/decisions/2026-08-22_重放連續潛水殘氮與前置判斷-設計.md`。⚠️ 收斂本身屬 DiveKit 範圍（單一戰場），Ultra 端採用是 App 層工作，由總指揮派工協調 |
