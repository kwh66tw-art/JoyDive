...
- **F5 已完成（2026-07-18）**：`JD2Core/Algorithm/` 的 DiveKit-fork（`Buhlmann`／
  ...
  改用統一 DiveKit 外部引用（`../../_JD2-family/DiveKit`，SPM local path，**目前 v1.27.3**）。
  **本 repo 已無任何演算法拷貝**。**2026-09-02修正**：本條原寫「`JD2Core/Algorithm/`
  ...
  `JD2Core/Algorithm/`現在只剩`DiveReplayInput.swift`（純輸入轉接，非重放邏輯
...
- **F6/F8/B組已完成（2026-07-19）**：Importers 全部（15 種格式）搬遷至獨立的
  `DiveImportKit`（`../../_JD2-family/DiveImportKit`，**目前 v0.7.0**），本 repo 已無任何本地
...
| 文件 | 用途 |
|------|------|
| `P0-00-文件登錄表_v1.10.md` | **全專案文件唯一索引**（狀態、版本、里程碑對照；編碼制），新增/更新文件時同步維護。舊版 `JD2-Logbook_文件登錄表_v1.7.md` 留存為歷史版本 |
